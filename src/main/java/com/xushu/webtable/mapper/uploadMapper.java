...
public interface uploadMapper {
    ...
    /**
     * 根据 MD5 查询文件是否存在,这个方法查到的永远只有一个原始文件
     */
    ...
    @Insert("insert into file " +
            "(file_name, original_file_name, user_id, file_size, path, create_time, ref_count,is_folder,parent_id)" +
            " values " +
            "(#{fileName}, #{originalFileName}, #{userId}, #{fileSize}, #{path}, #{createTime},#{refCount},#{isFolder},#{parentId})")
